...
open class CommandExecutor(private val plugin: ImprovedFactionsPlugin) : TabExecutor {
    ...
    fun registerCommandProcessor(processor: CommandProcessor) {
        if (commandProcessors.containsKey(processor.label)) {
            throw IllegalArgumentException(
                    "Command processor with label ${processor.label} already registered"
            )
        }
        ...
        commandProcessors =
                newProcessors.entries.sortedWith(
    ...
    override fun onTabComplete(
        ...
        val processor =
                findProcessor(joinedCommand) ?: return tabCompleteCommands(sender, originalArgs)
        ...
        val filteredResults =
                rawResults.filter { it.startsWith(originalArgs.last(), ignoreCase = true) }
    ...
    override fun onCommand(
        ...
        val result =
                runCatching { processor.execute(sender, args) }
                        .onFailure {
    ...
    private fun isProcessorAmbiguous(joinedCommand: String) =
            getPossibleProcessors(joinedCommand) > 1

    private fun getPossibleProcessors(arg: String) =
            commandProcessors.entries.count { it.key.contains(arg) }
