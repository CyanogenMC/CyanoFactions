...
class ImprovedFactionsConfig(
    ...
    override fun reload(plugin: ImprovedFactionsPlugin, config: FileConfiguration) {
        territoryDisplayLocation =
                config.getEnum<EventDisplayLocation>("event-display-location")
                        ?: territoryDisplayLocation
        ...
        hideWildernessTitle =
                config.getBoolean("factions.hide-wilderness-title", hideWildernessTitle)
        ...
        maxFactionIconLength =
                config.getInt("factions.unsafe.max-icon-length", maxFactionIconLength)
        ...
        hideDecorativeParticles =
                config.getBoolean(
                        "performance.decorative-particles.hidden",
                        hideDecorativeParticles
                )
        particleTickSpeed =
    ...
    companion object {
        fun createConfig(plugin: ImprovedFactionsPlugin): ImprovedFactionsConfig {
            ...
            plugin.logger.info(
                    "Config is outdated $configFileVersion -> $latestConfigVersion. Updating..."
            )
        ...
        fun generatePreviousValueTree(plugin: ImprovedFactionsPlugin) =
                plugin.config.getValues(true).filter { it.value !is MemorySection }.filter {
                    it.key != "config-version"
                }
        ...
        fun updateConfig(
            ...
            plugin.logger.info(
                    "${previousValueTree.size} values have been restored from the old config."
            )
        }
        ...
        private fun getLatestConfigStream() =
